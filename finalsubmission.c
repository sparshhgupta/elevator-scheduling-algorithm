...
int main() {
    ...
    if(pthread_mutex_init(&shm_mutex, NULL)!=0){
        printf("Failed to initialize shm mutex\n");
        return 1;
    }
    ...
    while(!msg.finished){
        ...
        if(msg.turnNumber>T){
            ...
            for(int i=0;i<movingElevatorsCnt;i++){
                int solverIndex=-1;
                while(solverIndex==-1){
                ...
                if(taskcnt>0){
                    if(pthread_create(&threads[solverIndex],NULL,guess,(void*)(intptr_t)solverIndex)!=0) {
